        ...
        switch (t->kind()) {
            ...
            default:
                throw SyntaxErrorException(
                        "Expected: geometry, camera, or light "
                        "information",
                        _tokenizer);
        }
    }
                        ...
                        const Material &mat)
{
    ...
    // Parse child geometry
    parseTransformableElement(
            scene, transform->createChild(glm::scale(glm::dvec3(x, y, z))),
            mat);
                           ...
                           const Material &mat)
{
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            case RBRACE:
                _tokenizer.Read(RBRACE);
                cylinder = new Cylinder(scene,
                                        newMat ? newMat : new Material(mat));
...
PointLight *Parser::parsePointLight(Scene *scene)
{
    ...
    for (;;) {
        ...
        switch (t->kind()) {
            ...
            default:
                throw SyntaxErrorException(
                        "expecting 'position' or 'color' "
                        "attribute, "
                        "or 'constant_attenuation_coeff', "
            ...
            case RBRACE:
                ...
                if (!hasPosition)
                    throw SyntaxErrorException("Expected: 'position'",
                                               _tokenizer);
                _tokenizer.Read(RBRACE);
                return new AreaLight(scene, position, color,
                                      constantAttenuationCoefficient,
                                      linearAttenuationCoefficient,
                                      quadraticAttenuationCoefficient,
                                      lightRadius);
