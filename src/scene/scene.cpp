...
bool Geometry::hasBoundingBoxCapability() const
{
    ...
    // ComputeBoundingBox() or the ComputeLocalBoundingBox() method must be
    // implemented.

    // If no bounding box capability is supported for an object, that object
    // will be checked against every single ray drawn.  This should be
    // avoided whenever possible, but this possibility exists so that new
    // primitives will not have to have bounding boxes implemented for them.
    return false;
}

std::vector<Geometry *> Geometry::getGeomElements()
{
...
bool Scene::intersect(ray &r, isect &i) const
{
    ...
    if(debugMode) std::cout << "Scene thinks intersect is " << have_one << std::endl;
...
void Scene::initKdTree(int maxDepth, int targSize)
{
    if (kdtree == nullptr) {
        std::vector<Geometry *> microObjs;
        for (const auto &obj : this->objects) {
            for (const auto &micro : obj->getGeomElements()) {
                microObjs.push_back(micro);
            }
        }
        kdtree = new KdTree<Geometry>(sceneBounds, microObjs);
        kdtree->build(maxDepth, targSize);
    }
}
