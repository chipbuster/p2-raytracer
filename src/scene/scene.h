...
class Geometry : public SceneElement {
    ...
    virtual std::vector<Geometry*> getGeomElements();
...
// Used to sort geometry. I controls the index to sort on
struct GeometrySorter{
    GeometrySorter(size_t i) : I(i) {} ;
    // Return true if A < B in ordering on dimension I
    inline bool operator()(const Geometry* a, const Geometry* b){
        ...
        glm::dvec3 centerA = (bba.getMax() + bba.getMin()) / 2.0;
        glm::dvec3 centerB = (bbb.getMax() + bbb.getMin()) / 2.0;
