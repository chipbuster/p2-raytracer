                                         ...
                                         const glm::dvec3 &p) const
{
...
#if defined(PATHTRACING) && defined(FORCEAREA) 
    ...
    for(int s = 0; s < lightSamples; s++) {
        ...
        double radius = MathUtil::randDouble(lightRadius); 
        glm::dvec3 shiftDir = glm::normalize(glm::cross(shadowDir, MathUtil::randHemisphere(shadowDir)));
        glm::dvec3 newPosition = position + shiftDir * radius;
        glm::dvec3 randShadowDir = glm::normalize(newPosition - p);
        ray sRay1(p - RAY_EPSILON * r.getDirection(), randShadowDir,
                  glm::dvec3(1.0), ray::SHADOW);
        isect i1;
        if (!scene->intersect(sRay1, i1)) {
            // No intersection: this ray is not attenuated
        ...
        if (glm::dot(i1.getN(), sRay1.getDirection()) > 0) {
            ...
            ray sRay2(isectPtMod, sRay1.getDirection(), glm::dvec3(1.0),
                      ray::SHADOW);
            res += atten * this->shadowAttenuation(sRay2, isectPtMod);
        } else {
            // This is an external collision. Fire an internal ray to find
            // the thickness of the object, then attenuate and find
            // remainder.
...
#ifdef PATHTRACING
    double lightSamples = scene->getConfig().getLightSamples();
    double theLightRadius = lightRadius;
    if(theLightRadius < 0.0) {
        theLightRadius = scene->getConfig().getLightRadius();
    }
    glm::dvec3 res(0.0);
    for(int s = 0; s < lightSamples; s++) {
        glm::dvec3 shadowDir = this->getDirection(p);
        double radius = MathUtil::randDouble(theLightRadius); 
        glm::dvec3 shiftDir = glm::normalize(glm::cross(shadowDir, MathUtil::randHemisphere(shadowDir)));
        glm::dvec3 newPosition = position + shiftDir * radius;
        glm::dvec3 randShadowDir = glm::normalize(newPosition - p);
        ray sRay1(p - RAY_EPSILON * r.getDirection(), randShadowDir,
