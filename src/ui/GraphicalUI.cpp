...
{
    ((GraphicalUI *)(o->user_data()))->m_nDepth =
...
void GraphicalUI::cb_render(Fl_Widget *o, void *v)
{
    ...
    if (pUI->raytracer->sceneLoaded()) {
        ...
        while (!pUI->raytracer->checkRender()) {
            ...
            t_elapsed = std::chrono::duration<double, std::ratio<1>>(t_now -
                                                                     t_start)
                                .count();
        ...
        if (pUI->aaSwitch() && !stopTrace) {
            ...
            auto t_total = std::chrono::duration<double, std::ratio<1>>(t_now -
                                                                        t_start)
                                   .count();
        ...
        /*
           ...
           min(100,(int)((double)(b_width*b_height*num_blocks)/(double)(width*height)*100.0));
                            ...
                            print(buffer, "%d%% of %s", frac,
           traceWindowLabel); pUI->m_traceGlWindow->label(buffer);
                        }
...
Fl_Menu_Item GraphicalUI::menuitems[] = {
        ...
        {"&Load Scene...", FL_ALT + 'l',
         (Fl_Callback *)GraphicalUI::cb_load_scene},
        {"&Load Cubemap...", FL_ALT + 'c',
         (Fl_Callback *)GraphicalUI::cb_load_cubemap},
        {"&Save Image...", FL_ALT + 's',
         (Fl_Callback *)GraphicalUI::cb_save_image},
...
GraphicalUI::GraphicalUI() : refreshInterval(10)
{
    ...
    m_thresholdSlider->user_data((void *)(this)); // record self to be used
                                                  // by static callback
                                                  // functions
    ...
    m_aaSamplesSlider->user_data((void *)(this)); // record self to be used
                                                  // by static callback
                                                  // functions
    ...
    m_aaThreshSlider = new Fl_Value_Slider(95, 237, 180, 20,
                                           "Supersample\nThreshold (x 0.001)");
    m_aaThreshSlider->user_data((void *)(this)); // record self to be used
                                                 // by static callback
                                                 // functions
    ...
    m_treeDepthSlider->user_data((void *)(this)); // record self to be used
                                                  // by static callback
                                                  // functions
    ...
    m_leafSizeSlider->user_data((void *)(this)); // record self to be used
                                                 // by static callback
                                                 // functions
